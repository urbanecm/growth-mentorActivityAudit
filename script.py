...
f.write('Username\tAverage response time (seconds)\tNumber of unanswered questions\n')
for mentor in mentors:
	print('Processing %s' % mentor)
	user_talk = pywikibot.Page(site, mentor, ns=3)
	code = mwparserfromhell.parse(user_talk.text)
	total_reaction = 0
	total_questions = 0
	...
	f.write('%s\t%d\t%d\n' % (mentor, total_reaction/total_questions, unanswered_questions))
